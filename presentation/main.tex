		...
		\begin{tikzpicture}[remember picture, overlay, shift={(current page.south west)}]
            ...
            \end{itemize}
				...
				\begin{itemize}
					...
					% \begin{itemize}
					%   \item used to record the first loopback 3-tuple observed for each unique attacker 3-tuple w
					% \end{itemize}
					% \item building independant pairs $\{(T_3, T_2), (T_3, T_5)\}$ for bucket of loopback 3-tupleS colliding via $G_{K_2}$ in the same perturbation table cell
				...
				%
				% \begin{itemize}
				%       \item device ID unique enough such that none of the other devices has same
				% \end{itemize}
			};
		\end{tikzpicture}
		\grid
