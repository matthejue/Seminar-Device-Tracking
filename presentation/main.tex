...
\ifattack
	...
	\begin{frame}[fragile]{Sandwiching Technique}{General principle}
		...
		\begin{tikzpicture}[remember picture, overlay, shift={(current page.south west)}]
			...
			\draw[thick, draw=PrimaryColor] (8.65cm,1.35cm) rectangle (9.6cm,1.75cm);
			\fill[white, opacity=0.95] (1cm,4.68cm) rectangle (14cm,7.2cm);
			\node[anchor=south west] at (2cm,5.2cm) {%
				\begin{adjustbox}{max width=0.7\textwidth}
					$\displaystyle
						table[i] =
						\begin{cases}
							rand(i)\, + 1, & \text{if } i \neq j,\ \text{meaning table cell } i \text{ was not used filling step} \\
							rand(i)\, + 2, & \text{if } i = j,\ \text{meaning table cell } i \text{ was used in filling step}
						\end{cases}
					$
				\end{adjustbox}
			};
		...
		\begin{tikzpicture}[remember picture, overlay, shift={(current page.south west)}]
			\node[anchor=north west, text width=5.5cm] at (8.5cm, 7cm) {%
				...
				\begin{itemize}
					...
					\item $S'$: \alert{Unique attacker 3-tuples} found so far%, maximal $T$ attacker 3-tuples) % quiz question!!!
					\item $P, P'$: Functions of $(IP_{Src}, IP_{Dest}, Port_{Dest})\mapsto Port_{Src}$ for \alert{new candiate attacker 3-tuples}
	...
	\begin{frame}[fragile]{Phase 1}{Pseudocode}
		...
		\begin{tikzpicture}[remember picture, overlay, shift={(current page.south west)}]
			\node[anchor=north west, text width=5.5cm] at (8.5cm, 7cm) {%
				...
				% \begin{itemize}
				% 	\item Candidate attacker 3-tuple only incremeted by 1
				% 	\begin{itemize}
				% 		\item[\alert{$\Rightarrow$}] Does \alert{not collide} with \alert{unique attacker 3-tuples} found so far, else increment $>1$
				% 		\item[\alert{$\Rightarrow$}] \alert{First} to map to a certain table cell
				% 		\item[\alert{$\Rightarrow$}] Is a \alert{unique attacker 3-tuple}
				% 	\end{itemize}
				% \end{itemize}
...
\ifattackphasetwo
	...
	\begin{frame}[fragile]{Phase 2}{Pseudocode}
		...
		\begin{tikzpicture}[remember picture, overlay, shift={(current page.south west)}]
			...
			% \draw[thick, draw=PrimaryColor] (1.9cm,3.75cm) rectangle (7.25cm,4.75cm);
			% condition
			\draw[thick, draw=PrimaryColor] (1.9cm,3.4cm) rectangle (5.15cm,3.75cm);
			\draw[->, thick, draw=PrimaryColor] (5.2cm,3.6cm) -- (7.6cm,3.55cm);
			% if
			% \draw[thick, draw=PrimaryColor] (1.9cm,1.75cm) rectangle (7.4cm,3.4cm);
			% \draw[thick, draw=PrimaryColor] (2.4cm,2.45cm) rectangle (4.95cm,3.1cm);
			% \draw[thick, draw=PrimaryColor] (2.4cm,1.75cm) rectangle (3.75cm,2.1cm);
		...
		\begin{tikzpicture}[remember picture, overlay, shift={(current page.south west)}]
			\node[anchor=north west, text width=6.5cm] at (7.5cm, 7.4cm) {%
				\begin{itemize}
					...
					\item S': Unique attacker 3-tuples from Phase 1
					\item w: The one unique attacker 3-tuple
					\item $\iota w$: \alert{Definite Description Operator}, \enquote{the unique w} attacker 3-tuple %satisfying the condition
				\end{itemize}
	...
	\begin{frame}[fragile]{Phase 2}{Influence of noise}
		...
		\begin{tikzpicture}[remember picture, overlay, shift={(current page.south west)}]
			...
			\node[anchor=south west] at (2cm,0.8cm) {%
				\begin{minipage}{\textwidth}
					\begin{itemize}
						\item \alert{Repeated connection attempts} with same loopback 3-tuple% to produce a detectable increment in its perturbation table cell
						\item[\alert{$\Rightarrow$}] Any increments caused by noise are \alert{outweighed}
					\end{itemize}
				\end{minipage}
			};
	...
	\begin{frame}[fragile]{Phase 2}{Pseudocode}
		...
		\begin{tikzpicture}[remember picture, overlay, shift={(current page.south west)}]
			...
			% \draw[->, thick, draw=PrimaryColor] (5.2cm,3.6cm) -- (7.6cm,4.1cm);
			% if
			\draw[thick, draw=PrimaryColor] (1.9cm,1.75cm) rectangle (7.35cm,3.4cm);
			\draw[thick, draw=PrimaryColor] (2.4cm,2.45cm) rectangle (4.95cm,3.1cm);
			\draw[thick, draw=PrimaryColor] (2.4cm,1.75cm) rectangle (3.75cm,2.1cm);
			\draw[->, thick, draw=PrimaryColor] (3.8cm,1.9cm) -- (7.6cm,2.4cm);
			% while loop
			% \draw[thick, draw=PrimaryColor] (1.3cm,1.4cm) rectangle (6.85cm,1.75cm);
			% \draw[thick, draw=PrimaryColor] (1.3cm,1cm) rectangle (2.9cm,1.4cm);
		\end{tikzpicture}
		\begin{tikzpicture}[remember picture, overlay, shift={(current page.south west)}]
			\node[anchor=north west, text width=6.5cm] at (7.5cm, 7.4cm) {%
				...
				\begin{itemize}
					\item $B[w]$: \alert{Dictionary}, keys = unique attacker 3-tuples, values = first loopback 3-tuples
					\item $\{T_3, T_2, T_5\}$ \alert{$\Rightarrow$} $\{(T_3, T_2), (T_3, T_5)\}$ % independent pairs 
				\end{itemize}
			};
		\end{tikzpicture}
		% \grid
	...
	\begin{frame}[fragile]{Phase 2}{Pseudocode}
		...
		\begin{tikzpicture}[remember picture, overlay, shift={(current page.south west)}]
			...
			% \draw[->, thick, draw=PrimaryColor] (3.8cm,1.9cm) -- (7.6cm,1.6cm);
			% while loop
			\draw[thick, draw=PrimaryColor] (1.3cm,1.4cm) rectangle (7.05cm,1.75cm);
			\draw[thick, draw=PrimaryColor] (1.3cm,1cm) rectangle (2.9cm,1.4cm);
		\end{tikzpicture}
		\begin{tikzpicture}[remember picture, overlay, shift={(current page.south west)}]
			\node[anchor=north west, text width=6.5cm] at (7.5cm, 7.4cm) {%
...
\ifappendix
  ...
  \subsection{Termination condition}
	...
	\begin{frame}[fragile]{Phase 2}{Termination condition}
		...
		\includegraphics[width=\textwidth, center]{./figures/external/termination_formula_explained.pdf}
		% \begin{itemize}
	...
	\begin{frame}[fragile]{Phase 2}{Termination condition}
		...
		\begin{itemize}
			...
			\begin{itemize}
				\item {\thickuparrow} $i$ \alert{$\Rightarrow$} $P_T(i, n)$ {\thickdownarrow}\, \alert{$\Rightarrow$} $P_T(i, n) < p^*$ potentially closer to being satisfied % closer to termination
				\vspace{-0.4cm}

				\includegraphics[width=0.95\textwidth]{./figures/external/increase_i.pdf} % ./figures/external/increase_i.tikz
				\item {\thickuparrow} $n$ \alert{$\Rightarrow$} $P_T(i, n)$ {\thickdownarrow}\, \alert{$\Rightarrow$} $P_T(i, n) < p^*$ potentially closer to being satisfied % closer to termination
				\vspace{-0.4cm}
