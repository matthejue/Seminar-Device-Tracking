...
% The previously mentioned tracking snippet operates in a website environment where JavaScript is restricted by the \alert{browser's sandbox} and has only limited access. From the attacker's perspective, the client device is a \alert{black box}, revealing only \alert{TCP SYN packets} whose headers expose the \alert{source port} chosen by DHPS to the tracking server. To observe these ports, the attacker must rely on \alert{attacker 3-tuples}, because only such connections generate TCP SYN packets visible to the server, while \alert{loopback 3-tuples} remain internal to the device and cannot be directly observed.

% The previously mentioned tracking snippet operates in a website environment where JavaScript is restricted by the \alert{browser's sandbox} and has only limited access. From the attacker's perspective, the client device is a \alert{black box}, revealing only \alert{TCP SYN packets} whose headers expose the \alert{source port} chosen by DHPS to the tracking server, but only for connections using \alert{attacker 3-tuples} and not for \alert{loopback 3-tuples}. To observe these ports, the attacker must rely on \alert{attacker 3-tuples}, because only such connections generate TCP SYN packets visible to the server, while \alert{loopback 3-tuples} remain internal to the device and cannot be directly observed.
% The previously mentioned tracking snippet operates in a website environment where JavaScript is restricted by the \alert{browser's sandbox} and has only limited access. From the attacker's perspective, the client device is a \alert{black box}, revealing only \alert{TCP SYN packets} whose headers expose the \alert{source port} chosen by DHPS to the tracking server, but only for connections using \alert{attacker 3-tuples} and not for \alert{loopback 3-tuples}. Although \alert{loopback 3-tuples} also generate TCP SYN packets internally, these packets never leave the device and are not accessible from outside, and due to the \alert{browser's sandbox} with its limited access, even the browser itself cannot directly observe these packets or the \alert{source ports} generated by DHPS for loopback connections.
% The previously mentioned tracking snippet operates in a website environment where JavaScript is restricted by the \alert{browser's sandbox} and has only limited access. From the attacker's perspective, the client device is a \alert{black box}, revealing only \alert{TCP SYN packets} whose headers expose the \alert{source port} chosen by DHPS to the tracking server, but only for connections using \alert{attacker 3-tuples}. This is because \alert{attacker 3-tuples} have the tracking server as their destination IP address, causing the SYN packets to traverse the network and become observable by the server. In contrast, \alert{loopback 3-tuples} use the client device itself as the destination IP, so although they also generate TCP SYN packets internally, these packets never leave the device and remain inaccessible from outside. Moreover, due to the \alert{browser's sandbox} and its limited access, even the browser cannot directly observe these internal packets or the \alert{source ports} generated by DHPS for loopback connections.
% The previously mentioned tracking snippet operates in a website environment where JavaScript is restricted by the \alert{browser's sandbox} and has only limited access. From the attacker's perspective, the client device is a \alert{black box}, revealing only \alert{TCP SYN packets} whose headers expose the \alert{source port} chosen by DHPS to the tracking server for connections using \alert{attacker 3-tuples}. These attacker 3-tuples have the tracking server as the destination IP, making their TCP SYN packets observable over the network. In contrast, \alert{loopback 3-tuples} use the client device itself as destination, so their TCP SYN packets remain internal and are inaccessible both to the tracking server and to the \alert{browser's sandbox}, which cannot access the \alert{source ports} generated by DHPS for such internal connections.
% The previously mentioned tracking snippet operates in a website environment where JavaScript is restricted by the \alert{browser's sandbox} and has only limited access. In contrast, \alert{loopback 3-tuples} use the client device itself as the destination IP address, so their TCP SYN packets remain entirely internal to the device and are inaccessible both to the tracking server and to the \alert{browser's sandbox}, which cannot access the \alert{source ports} generated by DHPS for such internal connections. Therefore, to make the source ports observable to the tracking server, the attack relies on \alert{attacker 3-tuples}, which have the tracking server as their destination IP address, causing their TCP SYN packets to traverse the network and reveal the \alert{source port} chosen by DHPS in their headers.
...
\begin{itemize}
    ...
    % \item \aalert{Multiple connection attempts:} Typically, multiple TCP connection attempts are established both in the wrapper and the filling steps, using several different 3-tuples. After applying the sandwiching technique, one knows which 3-tuples produced which source port values at specific points in time, because the \alert{TCP SYN packet headers} include the full attacker 4-tuple. This information can be easily transformed into a mapping from each attacker 3-tuple to its corresponding source port:
      \[
...
% At the end of the algorithm, the \alert{output} is a set of unique attacker 3-tuples. It is guaranteed that each cell in the perturbation table is covered by precisely one \alert{unique attacker 3-tuple}. Although the attacker does not know which specific \alert{index} in the table a particular attacker 3-tuple maps to, this one-to-one coverage is sufficient for the subsequent phase 2~\ref{sec:phase 2}.
...
% , and each unique attacker 3-tuple maps to exactly one cell in the perturbation table
% At the end of the algorithm, the \alert{output} is a set of unique attacker 3-tuples. It is guaranteed that each cell in the perturbation table is covered by precisely one \alert{unique attacker 3-tuple}. The attacker does not know which specific \alert{index} each attacker 3-tuple maps to in the table, but this lack of knowledge is immaterial to the attack because the existence of a one-to-one mapping suffices for the subsequent phases.

% Although it remains unknown which attacker 3-tuple maps to which specific index in the table, it is guaranteed that every
% perturbation table cell is covered by precisely one unique attacker 3-tuple
