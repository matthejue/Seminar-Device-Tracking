...
% In each iteration, the iteration counter is incremented, $i \leftarrow i+1$. If a collision is found and there is already a loopback 3-tuple $B[w]$ previously mapped to cell $w$, the pair $(L_i, B[w])$ is added to the set $C$ as an independent pair, and the counter is incremented, $n \leftarrow n+1$. Otherwise, $B[w]$ is set to $L_i$ to remember the first loopback 3-tuple for cell $w$.
...
% An attacker 3-tuple \(w\) is said to collide with a loopback 3-tuple if the following condition holds:
%
% \begin{equation*}
% 	\bigl(P'(w) - P(w)\bigr) > \varepsilon
% 	\quad \text{when sandwiching the loopback 3-tuple in the filling.}
% \end{equation*}
%
% Here, \(P(w)\) and \(P'(w)\) denote the observed source ports assigned to the attacker 3-tuple \(w\) during the first and second wrapper bursts, respectively, when the loopback 3-tuple is inserted into the filling.
%
% This means that \(w\) shares the same perturbation table cell as the loopback 3-tuple. Since only one loopback 3-tuple repeatedly establishes TCP connections during the filling, only one unique attacker 3-tuple \(w\) exhibits a significant increment greater than \(\varepsilon\), while all other attacker 3-tuples do not. Because each unique attacker 3-tuple corresponds to a distinct perturbation table cell (even if the exact cell is unknown), it becomes possible to identify which loopback 3-tuples collide with which unique attacker 3-tuples. In other words, one can determine for which unique attacker 3-tuples a loopback 3-tuple causes a significant increment.
...
Here, \(P(w)\) and \(P'(w)\) denote the observed \alert{source ports assigned to} the \alert{unique attacker 2-tuple \(w\)} during the first and third bursts in the wrapper, respectively, when the loopback 3-tuple \(L_i\) is inserted into the filling.
...
% \end{itemize}
%
% A loopback 3-tuple \(w\) collides with an attacker 3-tuple \(x\) if the following condition is satisfied during the sandwiching procedure:
%
% \begin{equation*}
%     \exists\, x \text{ such that } P'(x) - P(x) > \varepsilon
% \end{equation*}
%
% where \(P(x)\) and \(P'(x)\) denote the observed source ports assigned to attacker 3-tuple \(x\) in the first and second wrapper bursts, respectively, when the loopback 3-tuple \(w\) is placed in the filling.
%
% This means that \(w\) shares the same perturbation table cell as \(x\). Since only one loopback 3-tuple \(w\) repeatedly establishes TCP connections during the filling, only one unique attacker 3-tuple \(x\) experiences a significant increment greater than \(\varepsilon\), while all other attacker 3-tuples do not. Because each unique attacker 3-tuple uniquely represents a distinct perturbation table cell (even if it is unknown which specific cell), this makes it possible to determine which loopback 3-tuples collide in the same perturbation table cell as which unique attacker 3-tuples. In other words, one learns for which unique attacker 3-tuples a loopback 3-tuple \(w\) causes a significant increment.
%
...
% \end{itemize}
%
% A loopback 3-tuple collides with an attacker 3-tuple if the following condition is satisfied:
%
% \begin{equation*}
%     GK2(\text{IPSRC} = 127.0.0.1, \text{IPDST} = 127.1.2.3, \text{PORTDST} = x) 
%     = GK2(\text{IPSRC} = D, \text{IPDST} = A, \text{PORTDST} = y)
% \end{equation*}
%
% This condition implies that a loopback 3-tuple in the filling, through repeated TCP connection attempts, causes a significant increment greater than a threshold $\varepsilon$ to the counter associated with a unique attacker 3-tuple.
%
% Since only one loopback 3-tuple repeatedly establishes TCP connections, only one unique attacker 3-tuple experiences a significant increment greater than $\varepsilon$, while all others do not. This means that the loopback 3-tuple shares the same perturbation table cell as that unique attacker 3-tuple. Because each unique attacker 3-tuple uniquely represents a distinct perturbation table cell (even if it is unknown which specific cell), this makes it possible to determine which loopback 3-tuples collide in the same perturbation table cell as which unique attacker 3-tuples. In other words, one learns for which unique attacker 3-tuples a loopback 3-tuple causes a significant increment.
%
% Thus, it is possible to determine a structure of loopback 3-tuples that collide in the same perturbation table cell and use this structure as a device ID. This is achieved by collecting a set of independent pairs of loopback destination ports. The output of this process is a device ID composed of independent pairs of destination ports of loopback 3-tuples, which represent the structure of collisions within perturbation table cells.
%
% A significant increment greater than $\varepsilon$ is required to reliably detect collisions despite noise. The same unique attacker 3-tuple is contacted multiple times to produce a detectable increment in its perturbation table cell, ensuring that any increments caused by unrelated TCP connections on the device (noise) are outweighed.
%
% \section{Phase 2 old}
% \label{sec:phase 2 old}
%
