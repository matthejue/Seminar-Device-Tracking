...
%
% \begin{itemize}
%     \item \alert{Tracking server}: This server is controlled by the attacker and receives the TCP connection attempts sent from the client device. It collects and analyzes the observed TCP source ports in order to calculate a device ID and track the victim.
%
%     \item \alert{Client device}: This is the device running a browser which loads a web page containing a tracking snippet. The browser executes JavaScript code that establishes TCP connections both to the attacker's tracking server and to loopback addresses, enabling the tracking technique to reveal a unique device ID.
% \end{itemize}
%
In the attack, two main parties are involved:

\begin{itemize}
    \item \alert{Tracking server}: This server is controlled by the \alert{attacker} and receives the TCP connection attempts initiated from the client device. It analyzes the observed TCP source ports in order to calculate a device ID and track the victim.%across networks and browsing sessions.
