...
The \alert{term $\prod_{j=0}^{i - n - 1} \left(1 - \frac{j}{T}\right)$} accounts for the combined probability for another device to have all first loopback 3-tuples in their buckets to match the structure of the device at hand. The product goes from $0$ to $i - n - 1$, because $i - n$ is the \alert{number of non-empty buckets} of loopback 3-tuples colliding in the same perturbation table cell. One has to substract 1, because the indices start at $0$. 

In every iteration there's the possibility that a loopback 3-tuple is the first to map to a certain perturbation table cell und thus creating a new non-empty bucket. If a loopback 3-tuple is not the first and thus doesn't create a new non-empty bucket, then it collides with one of the first loopback 3-tuples. This means it is added to an already existing bucket and thus doesn't increase the number of non-empty buckets and thus has to be substracted from the number of iterations. This means one has to substract the number of loopback 3-tuple collisions / independant pairs $n$ from $l$ to get the number of non-empty buckets. 

The \alert{first choosen first loopback 3-tuple} of another device has a probabiliy of $\frac{T}{T} = 1$ to match the structure of the device at hand, it can basically choose any perturbation table cell. The \alert{second choosen first loopback 3-tuple} of another device has the probability of $\frac{T - 1}{T}$ to match the structure of the device at hand as it just has to pick another cell then the one chosen by the first chosen first loopback 3-tuple to be a first loopback 3-tuple and the exact perturbationt table cell is only known to the operating system of the client device but not known to the attacker. Thus for \alert{all first loopback 3-tuples} the probability is: $\frac{T}{T} \cdot \frac{T - 1}{T} \cdot \ldots \cdot \frac{T - (i - n - 1)}{T} = \prod_{j=0}^{i - n - 1} \left(\frac{T-j}{T}\right) = \prod_{j=0}^{i - n - 1} \left(\frac{T}{T} - \frac{j}{T}\right) = \prod_{j=0}^{i - n - 1} \left(1 - \frac{j}{T}\right)$.
...
Why the \alert{term $\binom{N}{2}$} accounts for the number of pairs of devices among the $N$ devices makes intuitively sense as soon as one knows that $\binom{N}{2}$ is the number of possible 2-tuples of devices among a set of $N$ devices.
