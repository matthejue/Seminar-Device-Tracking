...
\iftoggle{longversion}{Due to the Seminar's requirement that the report may not exceed \alert{four pages}, a more comprehensive version of this report is available at \url{https://github.com/matthejue/Seminar-Device-Tracking/releases/latest/download/report.pdf}.}{This document is the \alert{extended version} of a report created for a Seminar, which required the final submission to be limited to \alert{four pages}. It includes additional details and explanations that could not be included in the shorter Seminar report, which is not publicly available.}
